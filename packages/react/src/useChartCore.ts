...
export function useChartCore(
  ...
  const startIntervalMorph = useCallback(
    (h: VroomChartHandle, durationMs?: number) => {
  ...
  const startStreamAnim = useCallback(
    (h: VroomChartHandle, morphing: boolean, window: VisibleRange | null) => {
      ...
      // pinch, a wheel — lands somewhere different, which is how the slide
      // notices it is no longer the only thing moving the view and gets out of
      // the way. Cheaper than teaching every gesture to cancel it, and it can't
      // miss one.
  ...
  useEffect(() => {
    ...
    if (candles.length > 0) {
      ...
      if (freshHandle || prev.candles !== candles || prev.seriesKey !== seriesKey) {
        ...
        if (transition === 'stream' && prev != null && !explicit) {
          const { stream: mode, streamMs } = animRef.current;
          const stepMs = inferStepMs(candles);
          if (mode === 'transform' && streamMs > 0 && stepMs != null && !prefersReducedMotion()) {
  ...
  }, [ready, width, height, candles, showLoadingLine, seriesKey, explicit, startMs, endMs, defaultCandleWidth, themeKey, rsiKey, macdKey, atrKey, maKey, vwapKey, bollingerKey, ichimokuKey, fvgKey, volumeKey, drawingsKey, liquidityKey, priceLinesKey, footprintsKey, scheduleRender, startIntervalMorph, startLoadingHandoff, endIntervalMorph, startStreamAnim, settleStream]);
