...
A drawing is a small data-space object you can store however you like:

```ts
type Drawing = {
  id: string;                      // stable, generated by the chart
  type: 'line';
  points: [DrawPoint, DrawPoint];  // { timeMs, price } endpoints, in data space
  color?: VroomColor;              // default solid blue
  width?: number;                  // px, default 2
};
```

Because endpoints are `{ timeMs, price }` (not pixels), a saved drawing
re-renders in the right place across zoom, pan, and reloads. See
[`Drawing`](../reference/index.md) in the API reference.
